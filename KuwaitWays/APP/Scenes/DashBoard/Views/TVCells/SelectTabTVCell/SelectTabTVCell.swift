...
class SelectTabTVCell: TableViewCell {
   ...
   // ,"Private jet","Rent a car","Transfer","Events"]
...
extension SelectTabTVCell:UICollectionViewDelegate,UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if let cell = collectionView.cellForItem(at: indexPath) as? SelectTabCVCell {
            cell.imgHolderView.layer.borderColor = UIColor.AppBtnColor.cgColor
            cell.imgHolderView.layer.borderWidth = 2
            cell.tabImg.image = UIImage(named: tabImages[indexPath.row])?.withRenderingMode(.alwaysOriginal).withTintColor(.AppBtnColor)
